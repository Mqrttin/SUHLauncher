...
async init(config) {
    ...
    // Pausar cuando se lance Minecraft
    ipcRenderer.on("pause-audio", () => {
        if (this.music && !this.music.paused) this.music.pause();
    });

    // Reanudar cuando se cierre Minecraft
    ipcRenderer.on("resume-audio", () => {
        if (this.music && this.music.paused) this.music.play();
    });
    // --- FIN AGREGADO ---
...
async startGame() {
    ...
    // --- NUEVO: Pausar música antes de lanzar Minecraft ---
    ...
    // --- NUEVO: Reanudar música al cerrar Minecraft ---
    launch.on('close', code => {
        ipcRenderer.send('minecraft-close'); // <--- línea agregada
        if (configClient.launcher_config.closeLauncher == 'close-launcher') {
