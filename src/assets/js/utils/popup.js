    ...
    openPopup(info) {
        ...
        if (this.popupOptions.style.display !== 'none') {
            this.popupButton.addEventListener('click', () => {
                if (info.exit) {        
                    ipcRenderer.send('main-window-close'); // cierra ventana principal
                    ipcRenderer.send('tray-destroy');      // nueva señal para destruir tray
                    ipcRenderer.send('app-quit');          // nueva señal para cerrar app
                    return;  
                }
                this.closePopup();    
            })
